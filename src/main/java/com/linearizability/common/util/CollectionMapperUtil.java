...
public class CollectionMapperUtil {
     ...
     * @return 满足条件的字段值列表
     */
    public static <T, R> List<R> extractFieldListByCondition(List<T> list, Predicate<T> condition, Function<T, R> fieldExtractor) {
        if (list == null || list.isEmpty()) {
            return Collections.emptyList();
        }
        return list.stream()
     ...
     */
    public static <T, R> R extractFieldByCondition(List<T> list, Predicate<T> condition, Function<T, R> fieldExtractor) {
        if (list == null || list.isEmpty()) {
            return null;
        }
        return list.stream()
     ...
     * @return 分组后的Map
     */
    public static <T, K, V> Map<K, List<V>> extractAndGroupBy(List<T> list, Function<T, K> keyExtractor, Function<T, V> valueExtractor) {
        if (list == null || list.isEmpty()) {
     ...
     * @return 键值对Map
     */
    public static <T, K, V> Map<K, V> extractToMap(List<T> list, Function<T, K> keyExtractor, Function<T, V> valueExtractor) {
        if (list == null || list.isEmpty()) {
            return Collections.emptyMap();
        }
        return list.stream()
    ...
    public static <T, R> long countFieldValue(List<T> list, Function<T, R> fieldExtractor, R value) {
        ...
        return list.stream()
            .filter(Objects::nonNull)
            .map(fieldExtractor)
            .filter(fieldValue -> Objects.equals(fieldValue, value))
            .count();
    }
