...
public interface Converter<S, T> {
    ...
    default java.util.List<T> convertList(java.util.List<S> sources) {
        ...
        return sources.stream().map(this::convert).collect(java.util.stream.Collectors.toList());
    }
    ...
    default java.util.List<T> convertListNonNull(java.util.List<S> sources) {
        ...
        return sources.stream().map(this::convert).filter(java.util.Objects::nonNull)
                .collect(java.util.stream.Collectors.toList());
    }

    /**
     * 反向转换（可选实现） 默认抛出UnsupportedOperationException
     *
     * @param  target                        目标对象
     * @return                               源对象
