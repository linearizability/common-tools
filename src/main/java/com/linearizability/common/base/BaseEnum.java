...
public interface BaseEnum<V> {
    ...
    /**
     * 根据编码查找枚举
     * 支持多种匹配方式：== 比较、equals 比较、字符串忽略大小写比较
     *
     * @param type 枚举类型
     * @param code 编码
     * @param <T>  枚举类型
    ...
    static <T extends Enum<T> & BaseEnum<?>> Optional<T> findByCode(Class<T> type, Object code) {
        ...
        return find(type, e -> e.getCode() == code
            || (e.getCode() != null && e.getCode().equals(code))
    ...
    static <T extends Enum<T> & BaseEnum<?>> T findByCodeOrThrow(Class<T> type, Object code) {
        return findByCode(type, code).orElseThrow(() ->
            new IllegalArgumentException("No enum constant found with code: " + code + " in " + type.getName()));
    }
    ...
    static <T extends Enum<T> & BaseEnum<?>> T findByDescOrThrow(Class<T> type, String desc) {
        return findByDesc(type, desc).orElseThrow(() ->
            new IllegalArgumentException("No enum constant found with desc: " + desc + " in " + type.getName()));
    }
